...
import Styles from './receiptdetsilstimeline.Styles';
...
const Receiptsdetailstimeline = ({ navigation, route }) => {
    ...
    const categoryfoldercolour = '#' + route.params[0].category_color
    ...
    const openDialScreen = () => {
        let number = '';
        if (Platform.OS === 'ios') {
            number = 'telprompt:${route?.params[0]}';
        } else {
    ...
    return (
        <SafeAreaView style={Styles.container}>
            <ImageHeaderScrollView
                ...
                headerImage={{ uri: route?.params[0]?.product_image }}
                // headerImage={<Image source={route?.params[0]?.receipt_img} style={{ height: 20, width: Dimensions.get('window').width }} />}
                renderForeground={() => (
                    <View style={{ height: rh(8), flexDirection: "row", marginHorizontal: rw(2), width: rw(94), alignItems: 'center', justifyContent: 'space-between', bottom: rh(1), }} >
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: "space-between", width: rw(20) }}>
                            ...
                            <TouchableOpacity onPress={() => navigation.navigate('Edit', route?.params[0]?.ID)}>
                ...
                <ScrollView style={{ flexGrow: 1, backgroundColor: Colors.white, borderTopRightRadius: 24, borderTopLeftRadius: 24, marginBottom: rh(3) }}>
                    ...
                    <View style={{ height: rh(10), borderBottomWidth: 1, borderColor: Colors.gry, marginHorizontal: rw(4), flexDirection: 'row', justifyContent: 'space-between', alignItems: "center" }}>
                        <View style={{ flexDirection: 'row', alignItems: "center", justifyContent: 'space-between' }} >
                            ...
                            <Text style={{ fontSize: FontSize.font16, color: Colors.black, fontWeight: '700', fontFamily: 'Inter-Medium', left: rw(3) }}>{route?.params[0].category_name}</Text>
                        </View>
                        <View>
                            <Text style={{ fontSize: FontSize.font16, color: Colors.black, fontWeight: '500', fontFamily: 'Inter-Medium', marginTop: rh(1) }}>{route?.params[0]?.purchase_date}</Text>
                    ...
                    <View style={{ width: rw(100), borderBottomColor: Colors.gry, borderBottomWidth: 3, }}>
                        <View style={{ marginHorizontal: rw(4), }}>
                            <View style={{ backgroundColor: Colors.red, width: rw(25), alignItems: 'center', marginTop: rh(1.5), borderRadius: 4 }}>
                                <Text style={{ color: Colors.white, fontSize: FontSize.font12 }}>{route?.params[0]?.datediff} days left</Text>
                            </View>
                            <View style={{ marginTop: rh(3), marginBottom: rh(2.5) }}>
                                <Text style={Styles.boldheadding}>{route?.params[0]?.name}</Text>
                                <Text style={Styles.textsubhadding}>{route?.params[0]?.duration_years} Expires {route?.params[0]?.expiry_date}</Text>
                            </View>
                        </View>
                    ...
                    <View style={{ marginHorizontal: rw(4), borderBottomColor: Colors.gry, borderBottomWidth: 1 }}>
                        <View style={{ marginTop: rh(2) }}>
                            ...
                            <Text style={[Styles.lightbold,]}>{route?.params[0]?.model_number}</Text>

                            <Text style={[Styles.textsubhadding, { marginTop: rh(2) }]}>Price</Text>
                            <Text style={Styles.lightbold}>{route?.params[0]?.product_price}</Text>
                    ...
                    <View style={{ marginHorizontal: rw(4), borderBottomColor: Colors.gry, borderBottomWidth: 1, }}>
                        ...
                        <View style={{ flexDirection: "row", width: rw(80), justifyContent: 'space-between' }}>
                            <Text style={[Styles.lightbold,]}>{route?.params[0]?.purchase_date}</Text>
                            <Text style={[Styles.lightbold,]}>{route?.params[0]?.warranty_details_duration}</Text>
                        </View>
                        <View style={{ height: rh(2) }}>
                        </View>
                    </View>
                    <View style={{ marginHorizontal: rw(4), borderBottomColor: Colors.gry, borderBottomWidth: 1, }}>
                        ...
                        <View style={{ flexDirection: "row", width: rw(80), justifyContent: 'space-between' }}>
                            <Text style={[Styles.lightbold,]}>{route?.params[0]?.warranty_provider_name}</Text>
                            <Text style={[Styles.lightbold,]}>{route?.params[0]?.extended_warranty_duration}</Text>
                        </View>
                        <View style={{ height: rh(2) }}>
                        </View>
                    </View>
                    ...
                    <View style={{ marginHorizontal: rw(4), borderBottomColor: Colors.gry, borderBottomWidth: 1, }}>
                        ...
                        <Text style={[Styles.lightbold,]}>{route?.params[0]?.merchant_name}</Text>
                        <View>
                            <Text style={[Styles.textsubhadding, { marginTop: rh(2) }]}>Location</Text>
                            <Text style={[Styles.lightbold,]}>{route?.params[0]?.merchant_location}</Text>
                        </View>
                        ...
                        <View >
                            ...
                            <View style={{ flexDirection: "row", width: rw(90), justifyContent: 'space-between', alignItems: "center", alignContent: "center", }}>
                                <Text style={[Styles.lightbold,]}>{route?.params[0]?.merchant_contact_no}</Text>
                                <TouchableOpacity onPress={() => onPressMobileNumberClick(route?.params[0]?.merchant_contact_no)}>
                    ...
                    {route?.params[0]?.notes === '' ?
                        ...
                        <View View style={{ marginHorizontal: rw(4), borderBottomColor: Colors.gry, borderBottomWidth: 1, }}>
                            ...
                            <View style={{ paddingLeft: 10, paddingRight: 10, paddingBottom: 10 }}>
                                <Text style={[Styles.textsubhadding, { marginTop: rh(0), lineHeight: 19 }]}>{route?.params[0]?.notes}</Text>
                            </View>
                        ...
                        <TouchableOpacity style={{ alignSelf: 'center', marginTop: rh(3) }}>
                            <View style={{ justifyContent: "center", alignItems: "center" }}>
                                <AutoHeightImage
                                    ...
                                    source={{ uri: route?.params[0]?.product_image }}
                                />

                                {/* <Image source={{ uri: route?.params[0]?.product_image }} style={{ height: 500, width: 300, }} /> */}
                                {/* <Image source={{ uri: route?.params[0]?.receipt_img }} style={{ height: 500, width: 300, }} /> */}
                            </View>
                        </TouchableOpacity>
                    </View>
                        ...
                        <TouchableOpacity style={{ alignSelf: 'center', marginTop: rh(3) }}>
                            ...
                            <View style={{ justifyContent: "center", alignItems: "center" }}>
                                <AutoHeightImage
                                    ...
                                    width={300}
                                    style={{ borderRadius: 10 }}
                                    source={{ uri: route?.params[0]?.receipt_img }}
                                />
...
export default Receiptsdetailstimeline
