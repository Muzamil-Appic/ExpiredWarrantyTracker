...
const Addproductsteptwo = ({ navigation }) => {
    ...
    const uploadImage = async () => {
        ...
        if (Img != null) {
            ...
            const fileToUpload = Img;
            const data = new FormData();
            data.append('name', 'Image Upload');
            data.append('file_attachment', fileToUpload);
            // Please change file upload URL
            let res = await fetch(
                'http://waqarulhaq.com/expired-warranty-tracker/upload-img.php?',
                {
                    method: 'post',
                    body: data,
                    headers: {
    ...
    const selectimagefromgallery = () => {
        setTimeout(() => {
            ImagePicker.openPicker({
                ...
                let url = image.path;
                console.log(url);
                // let url = await uploadImage(image.path, 'Profile/' + ref);
    ...
    const uplodFile = async (a, b,) => {
        console.log("--------------->", a, b,);
        var formdata = new FormData();
        formdata.append("file", a, b);
        var requestOptions = {
            method: 'POST',
            body: formdata,
            redirect: 'follow'
        };
        fetch("http://waqarulhaq.com/expired-warranty-tracker/upload-img.php?", requestOptions)
    ...
    const opencamera = () => {
        ImagePicker.openCamera({
            ...
            uplodFile(image.mime, image.path,);

            //  console.log(url);
            // uploadImage()
            // await saveData('Users', user.uid, {profile: url})
            //   .then(() => {
