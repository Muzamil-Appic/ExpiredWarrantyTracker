...
const EditCategory = ({ navigation, route }) => {
    ...
    const [categoryname, setcategoryname] = useState(route?.params?.name)
    const [loader, setloader] = useState(false)
    const [categorykacolour, setcategorykacolour] = useState(route?.params?.color)
    ...
    const updatecategory = async () => {
        ...
        if (categorykacolour === '') {
            alert("Please Select Category Colout FIrst")
            return;
        ...
        fetch(`https://waqarulhaq.com/expired-warranty-tracker/edit-category.php?ID=${route?.params?.ID}&name=${categoryname}&color=${clr}`, requestOptions)
            .then(response => response.json())
            .then(result => {

                console.log(result);
                if (result.msg === "Category updated successfully!") {
                    empty()
                    navigation.goBack()
        ...
        <SafeAreaView style={styles.container}>
            <View style={{ flex: 1, marginHorizontal: rw(5) }}>
                ...
                <View style={{ marginTop: rh(2) }}>
                    <Text style={styles.secondhadding}>Update category</Text>
                </View>

                <Text style={styles.addproductparttext}>Category Name</Text>
                <View style={styles.addproductpartview}>
                    <TextInput value={categoryname} style={styles.addproductparttextinput} onChangeText={e => setcategoryname(e)} />
                </View>
                ...
                <View style={styles.nextanssavedbuttonview}>
                    {loader ?
                        ...
                        :
                        <TouchableOpacity style={styles.bottombtn} onPress={() => updatecategory()}>
                            <Text style={styles.bottombtntext}>Update</Text>
                        </TouchableOpacity>
                    }
                </View>
            </View>
